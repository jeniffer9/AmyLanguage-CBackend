  ...
  def run(ctx: Context)(v: (Program, SymbolTable)): Module = {
    ...
    def cgFunction(fd: FunDef, owner: Identifier, isMain: Boolean): Function = {
      ...
      Function(name, fd.params){
        ...
        val body = cgExpr(fd.body)(locals)
        if (isMain) {
    ...
    def cgExpr(expr: Expr)(implicit locals: Map[Identifier, Int]): Code = expr match {
      ...
      case Let(df, value: Expr, body: Expr) =>
        val localIndex = 0
        cgExpr(value) <:> SetLocal(localIndex) <:> cgExpr(body)(locals + (df.name -> localIndex))
      ...
      case Match(scrut: Expr, cases: List[MatchCase]) => {
        ...
        val matchCode = cases.map(c => {
          ...
          GetLocal(scrutId) <:> code <:> If_i32 <:> cgExpr(c.expr)(locals ++ newLocos) <:> Else
        })
